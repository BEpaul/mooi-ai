...
from prompt.defaults import DEFAULT_CHATBOT_PROMPT_MESSAGE, DEFAULT_SENTIMENT_PROMPT_MESSAGE
...
def run_streamlit_ui(chat_controller: ChatController):
    ...
    with st.sidebar:
        ...
        chat_prompt_message = st.text_area("상담사 프롬프트", value=DEFAULT_CHATBOT_PROMPT_MESSAGE)
        analyze_prompt_message = st.text_area("분석 프롬프트", value=DEFAULT_SENTIMENT_PROMPT_MESSAGE)
    ...
    if user_input := st.chat_input('당신의 마음을 표현하세요'):
        ...
        answer = chat_controller.generate_response(chat_prompt_message, chat_box.history, user_input)
